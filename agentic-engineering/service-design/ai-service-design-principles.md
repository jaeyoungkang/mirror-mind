...
AI가 동료 수준의 주체라면, 결과물도 동료 수준이어야 한다. 동료는 스프레드시트를 넘기지 않는다. 동료는 맥락에 맞게 구성된 **문서**를 작성한다.
...
1. **AI가 구조를 결정한다** — 결과물의 레이아웃과 서사 구조를 개발자가 미리 정의하지 않는다. LLM이 현재 맥락(시나리오, 작업 단계, 사용자 상태)에 맞게 문서를 구성한다. 같은 데이터라도 시나리오에 따라 완전히 다른 문서가 나올 수 있다.

2. **마크다운이 범용 포맷이다** — 구조화된 문서의 기본 포맷으로 마크다운을 사용한다. 테이블, 목록, 제목, 강조 등 마크다운의 표현력이 대부분의 결과물에 충분하다. 특수한 인터랙티브 요소는 문서 안에 슬롯으로 삽입한다.

3. **문서는 점진적으로 풍부해진다** — 한 번 생성한 문서를 작업 진행에 따라 갱신한다. 초기 결과에 후속 분석이 추가되고, 구조화가 더해진다. 문서가 작업의 축적을 반영한다.

4. **인터랙티브 슬롯은 최소한으로** — 마크다운 안에 인터랙티브 컴포넌트를 삽입할 수 있으나, 필요한 것만 점진적으로 추가한다. 순수 마크다운으로 시작하고, 실제 사용에서 부족함이 확인된 것만 인터랙티브로 전환한다.

### 적용 방법

- 결과물 표시 영역을 고정 컴포넌트가 아닌 마크다운 렌더러로 구현한다
- LLM에게 결과물 작성 도구를 부여하여, 문서의 생성과 갱신을 AI가 직접 수행한다
- 시스템 프롬프트에 시나리오별 문서 구조 가이드를 포함하되, 엄격하게 강제하지 않는다
- 작업이 진행되면서 문서가 갱신되는 흐름을 설계한다

> 출처: Light House 프로젝트에서 외부 벤치마크 분석을 통해 도출. 고정 React 컴포넌트로 데이터를 표시하는 방식이 시나리오별 다른 표현을 불가능하게 만드는 한계를 발견. LLM의 동적 문서 생성 능력을 활용하여 패러다임 전환.
