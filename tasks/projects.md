...
- 하위 업무:
  ...
  - [>] 새학기 프로파일링 — MBTI 기반 프로파일 카드 + 공유 바이럴 `재영`
    - 기획서: `projects/moonlight/fortune-feature-spec.md`
    - 카드 업데이트: `projects/moonlight/fortune-card-update-prompt.md`
    - 카드 샘플: `projects/moonlight/fortune-card-samples.html`
    - 핵심: 참여=응원+1, 공유링크 유입(대학계정)=공유자 응원+1
    - 바이럴 루프: 부탁→공유 구조 전환, 카드 OG 이미지
    - [x] 기획서 작성
    - [x] 1차 구현 + 배포
    - [x] 카드 비주얼 설계 — 4기질별 배경 패턴 + 행운 요소 6종 풀(2개 선택) + OG 이미지
    - [x] OG 이미지 Satori 에러 해결
    - [x] 레퍼럴 응원 GAS 일원화 — referral_cheers 테이블 제거, 로그인 유저 이메일로 cheer, 리다이렉트 유지
    - [x] 날짜 타임존 수정 + 타이틀 "문라이트의 새학기 프로파일링" 통일
    - [>] 프로파일 자동 생성 테스트 — 카드 생성 시 프로파일 미존재 유저 자동 생성(ensureProfile) 검증
