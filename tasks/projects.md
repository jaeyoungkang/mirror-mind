...
- 목표: 회원가입 확대 → **구독 전환**(최종). 이벤트는 수단
- 도메인: 서비스 운영, 마케팅
- 팀: 재영, 예린, 은솔
- 마감: 3/13 (수정 예정, 페이지는 아직 3/27)
- 상세: `projects/moonlight/university-event.md`
- 현황 (2/26 CSV): 프로필 79명, 응원 96개, 응원 0명 56명(71%), 일 방문 ~400명
- 핵심 문제: 71%가 0명 — "만들면 알아서 온다" 오해 가능성. 홍보 가이드 + 넛지 필요
- 투트랙 전략:
  - A. 기존 참여자 → 직접 홍보 유도 (가이드, 상위권 꿀팁)
  - B. 신규 유입 → 게임/콘텐츠(운세, 단어게임)로 끌어오기 → 대학 메일 가입 → 응원
- 하위 업무:
  - [ ] 퍼널 다듬기 (이벤트 버튼, CTA→프로필 생성, 응원 모달 공개) `은솔`
  - [ ] 유저 소통 (웃긴 메시지 활용, 상위권 협력자 만들기) `예린`
  - [ ] 0명 유저 동기부여 (상위권 꿀팁 정리→발송, 홍보 가이드) `예린`
  - [ ] 외부 홍보 (인스타/스레드 콘텐츠, 광고, 인플루언서) `예린`
  - [x] 나의 학기 사용설명서 — MBTI 기반 5챕터 시스템 `재영`
    - 배경: 기존 "학업 운세" 단일 콘텐츠의 흥미도 부족 → 5가지 포맷으로 확장
    - 카드 샘플: `projects/moonlight/chapter-card-samples.html` (학부 ENFP), `chapter-card-samples-grad.html` (대학원 INTJ)
    - 콘셉트: 매일 1챕터 언락(5일 완성), 프로그레스바+잠금 리스트, 인앱 MBTI 숨김/공유 카드에만 표시
    - 5챕터: 생존 확률 / 학기 스탯 / 금지어 / 시나리오 / 교수님 궁합
    - 핵심: 공유 유입 가입 시에만 공유자 응원+1 (생성 시 자동 응원 없음)
    - [x] 콘텐츠 방향 재설계 — 5가지 변주 + "학기 사용설명서" 브랜딩
    - [x] Gemini 3 Flash 샘플 생성 + 평가 (학부/대학원 각 1세트)
    - [x] 카드 시각화 — moonlight 디자인 시스템 반영, 선택 화면 + 인앱 카드 + 공유 카드
    - [x] 구현 프롬프트 작성 — 서버/프론트/OG/응원 전체 명세
    - [x] 구현 + 안정화 — 서버/프론트/OG/FAQ 전체 완료, 비로그인 챕터 표시, 주사위 애니메이션
    - [x] 공유 OG 연결 — 챕터별 OG 메타태그 + 카드 이미지 동적 생성
    - [x] 프로덕션 안정화 — 하루 1회 제한·챕터 언락 복원, 응원 로직 변경(공유 유입만), 카드 API 개인정보 필터링, GAS timeout·재시도, LLM 에러 피드백, 프론트 에러 핸들링
    - [ ] 로컬 통합 테스트 (언락, 하루 제한, 공유 OG, 에러 UI)
    - [ ] 프로덕션 배포 (DB 마이그레이션, 프로덕션 fortune 데이터 없음 확인 완료)
  - [ ] 게임 연동 — 기존 자산 활용, 완수→응원 1카운트 `재영`
  - [ ] 인사이트 에이전트 — CSV/DB/노션을 LLM이 읽고 분석, 웹 대화형 `재영`
  - [ ] 마감 기한 3/13 수정 + D-14일 변경 안내 `은솔`

## [x] OpenClaw Docker + Discord 설치
- 목표: OpenClaw을 Docker에 설치하고 Discord 봇으로 연동
- 도메인: 인프라
- 가이드: `projects/openclaw/setup-guide.md`
- 하위 업무:
  - [x] Docker 설치 + onboarding
  - [x] Discord 봇 생성 + 서버 초대 + Intent 설정
  - [x] openclaw.json 채널/서버 설정
  - [x] 페어링 승인
  - [x] Google Gemini API 키 + auth-profiles.json 설정
  - [x] 정상 동작 확인 (gemini-3-flash-preview)
- 향후:
  - [ ] ChatGPT OAuth 인증을 Docker에 전달하는 방법 조사
