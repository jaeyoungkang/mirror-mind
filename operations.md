...
- 현황 조회: `gh issue list --repo jaeyoungkang/mirror-mind --label <라벨> --state open`

### 업무 수집
- 대화 중 업무 식별 시 AI가 선제적으로 등록 제안
- 합의 후 GitHub Issue 생성 (`gh issue create`)
...
### 업무 구조화
- 라벨(프로젝트)로 분류. 이슈 본문에 체크리스트로 하위 업무 표현
- 문서 분리/통합 기준: (1) 독립적 의미 (2) 추상화로 흡수 가능. 둘 다 아니면 통합

### 진행 관리
- GitHub Issues 상태: open (대기/진행) / closed (완료)
- GitHub Projects 보드로 칸반 관리 (Backlog → In Progress → Done)

### 일정
- 마일스톤은 GitHub Milestones 또는 이슈에 날짜 기록
...
| 트리거 | 절차 |
|--------|------|
| **중간 정리** | 체크포인트 파일 작성 → close-session.py --checkpoint (노드 추출 + 오프셋 기록) → 커밋 → `/clear` → `계속` 입력 → 체크포인트 읽고 이어서 작업 |
| **주제 전환** / **킵** | 메타에이전트 리포트 확인 → 중간 상태를 해당 GitHub Issue에 코멘트 → 전환 |
| **정합성 검증** | 전체 원칙·설계 문서를 읽고 상호 참조·일관성 검증 리포트 |
| **회고** | 마일스톤 단위 회고, 교훈을 원칙 문서에 반영 |
| **원칙 점검** | `python scripts/check.py` 1회 실행 |

### 주제 전환 규칙
- 전환 전 현재 진행 상태를 해당 GitHub Issue에 코멘트
- 복귀 시 이슈 코멘트 + 체크포인트 참조하여 맥락 복원
...
| 프로젝트 상태 정합성 | GitHub Issues 상태 ↔ 실제 진행 상태 | warning |
